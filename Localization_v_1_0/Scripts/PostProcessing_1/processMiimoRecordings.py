...
def parseLog(fh):
    ...
    for n in range(N):
        ar[n, :] = odo[n]
    return imageNameTime, ar, odoMessageList
...
def writeImageTime(workPeriod, imageNameTime, inDir, workPeriodCnt):
    # write coordinates csv file with estimated positions of images
    fh = open("%s/coordinates_wp%d.txt" % (inDir, workPeriodCnt), 'wt')
    ...
    for fn, t in imageNameTime:
        if workPeriod[0, 0] <= t <= workPeriod[-1, 0]:
            ...
            fh.write("%.4f,%.4f,%s,%.4f,%.4f,%.4f\n" % (t, t_capture, imgName, err, x, y))
...
if __name__ == "__main__":
    ...
    mapPoints = loadMap('%s/map.PKT'%inDir)	# look for ground truth map data for visualization (and plausibilization)

    #  For indoor area
    # mapPoints = loadMapIndoor('%s/indoorMap.csv'%inDir)  # 'indoorMap.csv'
    # # Only for indoor map
    # mapPoints[:, 0] -= 0.0030
    # mapPoints[:, 1] -= 0.4400
    ...
    imageNameTime, odoArr, odoMessageList = parseLog(fh)
    ...
    for cnt in range(len(workTracks)):
        ...
        picPosArr, travelledDist, picPosLst = writeImageTime(camOffsetTrack, imageNameTime, inDir, cnt)
        ...
        pylab.plot(workTracks[cnt][:, 1], workTracks[cnt][:, 2])
