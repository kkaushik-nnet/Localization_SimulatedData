...
int main(int argc, char **argv)
{
    ...
    for (size_t i = 0; i < image_file_names.size(); ++i){
        ...
        float total_count = image_file_names.size();
        progress = (i/total_count)*100;
        prev_progress = (i-1)*100/total_count;
       // cout << int(progress) <<" : " << int(prev_progress) << endl;
        if(int(progress)==1 && (int(progress)>int(prev_progress)))
        {
        cout <<" "<< int(prev_progress) << "% [====================";
        }
        if ((remainder(int(progress),10)==0) && (int(progress)>int(prev_progress)) && (int(progress) > 9))
        {
          cout << "> ] "<< endl;
          cout <<  int(progress) << "% [";
        }
        if (int(progress)>int(prev_progress) && int(progress)>1)
        {
           cout <<  "==========";
        }
        if (i == image_file_names.size()-1){
           cout << "> ]";
        }
