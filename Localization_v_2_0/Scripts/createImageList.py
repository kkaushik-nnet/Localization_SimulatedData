...
def create_image_list(path_directory, output_file, image_suffix):
    ...
    # create an array of all files in directory with image suffix image_files = np.array([f for f in os.listdir(
    # path_directory) if f.startswith('panorama_') and f.endswith(image_suffix)])
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        ...
        print("\n Example2 : python createImageList.py '/hri/localdisk/ThesisProject/Kaushik/Simulator_data/"
              "SingleMarker/train/' '/hri/localdisk/ThesisProject/Kaushik/Version 1.1/Landmark_based_localization/"
              "Output/trainList.xml' '.png'\n")
        sys.exit()
